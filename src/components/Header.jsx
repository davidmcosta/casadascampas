  ...
  <div className="max-w-7xl mx-auto px-3 sm:px-4 md:px-6 lg:px-8 xl:px-10">
    <div className="flex items-center justify-between h-14 sm:h-16 lg:h-18">
      ...
      <nav className="hidden lg:flex items-center space-x-6 xl:space-x-8">
        {navLinks.map(link => (
          <Link
            key={link.href}
            href={link.href}
            className={`text-sm tracking-wide transition-all duration-300 pb-1 px-2 py-1 hover:scale-105 border-b-2 ${pathname === link.href ? "text-amberVar border-amberVar" : "text-white border-transparent hover:text-amberVar hover:border-amberVar"}`}
          >
            {link.label}
          </Link>
        ))}
      ...
      <div className="flex items-center lg:hidden space-x-3">
        ...
        <button
          onClick={toggleCart}
          className="relative p-2 text-white hover:text-amberVar transition-all duration-300 hover:bg-blackVar/50 rounded-lg"
          aria-label="Shopping cart"
        >
